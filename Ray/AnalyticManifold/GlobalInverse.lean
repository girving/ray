...
theorem global_complex_inverse_fun_open {f : ℂ → S → T} [Nonempty S] {s : Set (ℂ × S)}
    (fa : MAnalyticOn II I (uncurry f) s) (nc : ∀ p : ℂ × S, p ∈ s → mfderiv I I (f p.1) p.2 ≠ 0)
    (inj : InjOn (fun p : ℂ × S ↦ (p.1, f p.1 p.2)) s) (so : IsOpen s) :
    ∃ g : ℂ → T → S,
      MAnalyticOn II I (uncurry g) ((fun p : ℂ × S ↦ (p.1, f p.1 p.2)) '' s) ∧
  ...
  have ge : ∀ (p : ℂ × S) (m : p ∈ s), ∀ᶠ q : ℂ × T in 𝓝 (p.1, f p.1 p.2),
    ...
    have n := nontrivialMAnalyticAt_of_mfderiv_ne_zero (fa _ m).along_snd (nc _ m); simp only at n
...
theorem global_complex_inverse_fun_compact {f : ℂ → S → T} [Nonempty S] [T2Space T]
    {s : Set (ℂ × S)} (fa : MAnalyticOn II I (uncurry f) s)
  ...
  set u := t ∩ {p | MAnalyticAt II I (uncurry f) p ∧ mfderiv I I (f p.1) p.2 ≠ 0}
  ...
  have fa' : MAnalyticOn II I (uncurry f) u := fun _ m ↦ (inter_subset_right m).1
...
theorem weak_global_complex_inverse_fun_open {f : S → T} [Nonempty S] {s : Set S}
    (fa : MAnalyticOn I I f s) (nc : ∀ z, z ∈ s → mfderiv I I f z ≠ 0) (inj : InjOn f s)
    (so : IsOpen s) : ∃ g : T → S, MAnalyticOn I I g (f '' s) ∧ ∀ z, z ∈ s → g (f z) = z := by
  ...
  have fa' : MAnalyticOn II I (uncurry f') (univ ×ˢ s) := by
    intro ⟨c, z⟩ ⟨_, zs⟩; exact (fa z zs).comp_of_eq mAnalyticAt_snd rfl
...
theorem global_complex_inverse_fun_open' {f : S → T} [Nonempty S] {s : Set S}
    (fa : MAnalyticOn I I f s) (inj : InjOn f s) (so : IsOpen s) :
    ∃ g : T → S, MAnalyticOn I I g (f '' s) ∧ ∀ z, z ∈ s → g (f z) = z :=
