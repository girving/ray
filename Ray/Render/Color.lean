...
def color_error : ℚ := 3 / 256

/-- Halved approximation radius -/
def half_color_error : Interval := .ofRat (color_error / 2)
...
def Color.approx' [Approx α β] (c : Color α) : Set (Color β) :=
  {e : Color β | e.r ∈ approx c.r ∧ e.g ∈ approx c.g ∧ e.b ∈ approx c.b ∧ e.a ∈ approx c.a}

/-- `Color UInt8` approximates componentwise, after a single nan -/
...
@[simp] lemma UInt8.val_unquantize {v : UInt8} (n : v.unquantize ≠ nan) :
  ...
  simp only [Floating.val_scaleB n, Floating.val_ofNat' lt, e9, Nat.cast_add, Nat.cast_mul,
    Nat.cast_ofNat, Nat.cast_one, Int.reduceNeg, zpow_neg, add_mul, one_mul, one_div]
  ring_nf
...
@[approx] lemma Interval.mem_approx_quantize {x' : ℝ} {x : Interval} (xm : x' ∈ approx x) :
  ...
  · have xn : x ≠ nan := by
    ...
    simp only [g, and_self, ↓reduceIte, approx_some, approx, Rat.cast_div, Rat.cast_ofNat, mem_Icc,
      tsub_le_iff_right, UInt8.color, add_div]
...
def Color.ofRat (c : Color ℚ) : Color Interval :=
  c.map .ofRat
...
noncomputable instance : ApproxNeg (Color Interval) (Color ℝ) where
  approx_neg x := by
    ...
    simp only [Color.neg_def] at m ⊢
    simp only [approx]
    simp only [Color.approx', mem_setOf_eq, Interval.approx_neg, mem_neg]
    exact m
...
noncomputable instance : ApproxSub (Color Interval) (Color ℝ) where
  approx_sub x y := by
    ...
    rcases m with ⟨x',xm,y',ym,e⟩
    simp only [Color.sub_def, smul_eq_mul, approx, ←e] at xm ym ⊢
    simp only [Color.approx', mem_setOf_eq] at xm ym ⊢
    rcases xm with ⟨x0, x1, x2, x3⟩
    rcases ym with ⟨y0, y1, y2, y3⟩
    refine ⟨?_, ?_, ?_, ?_⟩
