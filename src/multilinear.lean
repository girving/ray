...
def fst_cmmap (R : Type) (A B : Type) [semiring R]
  ...
  to_fun := λ z, (z 0).fst,
  map_add' := begin intros d z i x y, have i0 : i = 0 := by simp, rw i0, simp end,
  map_smul' := begin intros d z i s x, have i0 : i = 0 := by simp, rw i0, simp end,
  cont := by continuity,
}
...
lemma fst_cmmap_norm [normed_ring A] [normed_algebra 𝕜 A] [norm_one_class A] [normed_ring B] [normed_algebra 𝕜 B] [norm_one_class B]
  ...
  apply le_antisymm, {
    apply @continuous_multilinear_map.op_norm_le_bound 𝕜 _ (λ _ : fin 1, A × B) _ _ _ _ _ _ _ (fst_cmmap 𝕜 A B) (1 : ℝ) (by norm_num),
    intro z, simp,
    ...
    have lo := @continuous_multilinear_map.unit_le_op_norm 𝕜 _ (λ _ : fin 1, A × B) A _ _ _ _ _ _ (fst_cmmap 𝕜 A B) (λ _, (1,1)) _,
    rw [fst_cmmap_apply, norm_one] at lo, assumption,
    rw pi_norm_le_iff_of_nonneg, intro i, simp only [prod.norm_def, norm_one, max_eq_right], norm_num,
  },
end
...
lemma snd_cmmap_norm [normed_ring A] [normed_algebra 𝕜 A] [norm_one_class A] [normed_ring B] [normed_algebra 𝕜 B] [norm_one_class B]
  ...
  apply le_antisymm, {
    apply @continuous_multilinear_map.op_norm_le_bound 𝕜 _ (λ _ : fin 1, A × B) _ _ _ _ _ _ _ (snd_cmmap 𝕜 A B) (1 : ℝ) (by norm_num),
    intro z, simp,
    have e : z = (λ _, ((z 0).fst, (z 0).snd)), { apply funext, intro i, rw fin.eq_zero i, simp }, rw e,
    rw snd_cmmap_apply, simp, exact norm_snd_le (z 0)
  }, {
    have lo := @continuous_multilinear_map.unit_le_op_norm 𝕜 _ (λ _ : fin 1, A × B) B _ _ _ _ _ _ (snd_cmmap 𝕜 A B) (λ _, (1,1)) _,
...
lemma update_0_succ (d : decidable_eq (fin (n+1))) (f : fin (n+1) → A) (x : A) (i : fin n)
    : @function.update _ _ d f 0 x i.succ = f i.succ := begin
  rw function.update_apply, simp,
  have i0 := fin.succ_ne_zero i,
  finish,
end
lemma update_nz_0 (d : decidable_eq (fin (n+1))) (f : fin (n+1) → A) {x : A} {i : fin (n+1)} (i0 : i ≠ 0)
    : @function.update _ _ d f i x 0 = f 0 :=
  by rw function.update_noteq i0.symm
lemma update_nz_succ (d : decidable_eq (fin (n+1))) (f : fin (n+1) → A) (x : A) {i : fin (n+1)} (i0 : i ≠ 0)
    : (λ j : fin n, @function.update _ _ d f i x j.succ) = function.update (λ j : fin n, f j.succ) (i.pred i0) x := begin
...
lemma smul_cmmap_add [add_comm_monoid A] [module 𝕜 A] [topological_space A] [normed_add_comm_group B] [normed_space 𝕜 B]
    (x : continuous_multilinear_map 𝕜 (λ _ : fin 1, A) 𝕜)
    (xs : continuous_multilinear_map 𝕜 (λ _ : fin n, A) B)
    : ∀ (d : decidable_eq (fin (n+1))) (z : Π _ : fin (n+1), A) (i : fin (n+1)) (u v : A),
      smul_cmmap_fn x xs (@function.update _ _ d z i (u + v)) =
      smul_cmmap_fn x xs (@function.update _ _ d z i u) + smul_cmmap_fn x xs (@function.update _ _ d z i v) := begin
  intros d z i u v,
  rw smul_cmmap_fn,
  by_cases i0 : i = 0, {
    rw i0, simp, simp_rw update_0_succ d z _ _,
    ...
    simp,
    simp_rw update_nz_0 d z i0,
    rw update_nz_succ d z _ i0,
    rw update_nz_succ d z _ i0,
    rw update_nz_succ d z _ i0,
...
lemma smul_cmmap_smul [add_comm_monoid A] [module 𝕜 A] [topological_space A] [normed_add_comm_group B] [normed_space 𝕜 B]
    (x : continuous_multilinear_map 𝕜 (λ _ : fin 1, A) 𝕜)
    (xs : continuous_multilinear_map 𝕜 (λ _ : fin n, A) B)
    : ∀ (d : decidable_eq (fin (n+1))) (z : Π _ : fin (n+1), A) (i : fin (n+1)) (s : 𝕜) (u : A),
      smul_cmmap_fn x xs (@function.update _ _ d z i (s • u)) =
        s • smul_cmmap_fn x xs (@function.update _ _ d z i u) := begin
  intros d z i s u,
  rw smul_cmmap_fn,
  by_cases i0 : i = 0, {
    rw i0, simp, simp_rw update_0_succ d z _ _,
...
| 0 := λ _ x, continuous_multilinear_map.const_of_is_empty _ _ x
...
lemma term_cmmap_apply [normed_add_comm_group E] [normed_space 𝕜 E] [smul_comm_class 𝕜 𝕜 E] [is_scalar_tower 𝕜 𝕜 E]
  ...
  induction n with n h, {
    simp only [term_cmmap, continuous_multilinear_map.const_of_is_empty_apply, min_zero, pow_zero, zero_tsub, one_smul],
  }, {
...
lemma term_cmmap_norm (𝕜 : Type) [nontrivially_normed_field 𝕜] [normed_add_comm_group E] [normed_space 𝕜 E]
  ...
  induction n with n nh, {
    simp only [term_cmmap, @continuous_multilinear_map.norm_const_of_is_empty 𝕜 _ (λ _ : fin 0, 𝕜 × 𝕜) E _ _ _ _ _ _ _ x],
  }, {
    rw term_cmmap, simp only,
...
-- conj as a continuous_linear_map. This is roughly starₗᵢ ℂ, but that has a much more complicated type.
