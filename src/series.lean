...
lemma uniform_vanishing_to_tendsto_uniformly_on {f : ℕ → ℂ → ℂ} {s : set ℂ}
  ...
  ... ≤ e/4 + (M \ N).sum (λ n, abs (f n z)) : by bound [finset_complex_abs_sum_le (M \ N) (λ n, f n z)]
...
lemma c_nonpos.degenerate {f : ℕ → ℂ → ℂ} {s : set ℂ} {c a : ℝ}
  ...
  exact complex.abs.eq_zero.mp (le_antisymm (trans hf ca) (complex.abs.nonneg _))
end
...
theorem fast_series_converge_uniformly_on {f : ℕ → ℂ → ℂ} {s : set ℂ} {c a : ℝ}
  ...
  by_cases c0 : c ≤ 0, {
    have fz := c_nonpos.degenerate c0 a0 hf, simp only at fz,
    rw [has_uniform_sum, metric.tendsto_uniformly_on_iff],
    intros e ep, apply filter.eventually_of_forall, intros n z zs,
    rw tsum_on, simp only,
    simp_rw fz _ z zs,
    simp only [tsum_zero, finset.sum_const_zero, dist_zero_left, complex.norm_eq_abs, absolute_value.map_zero],
    assumption,
  }, {
    ...
    ... = 1 * 1 * (e / 2) : by rw [field.mul_inv_cancel (ne_of_gt a1p), div_self (ne_of_gt c0)]
    ... = e / 2 : by ring
    ... < e : by bound,
  },
end
...
lemma finite_sums_are_analytic {f : ℕ → ℂ → ℂ} {s : set ℂ}
  ...
  induction N using finset.induction with a B aB hB, {
    ...
    intros z zs, simp_rw finset.sum_insert aB, exact (h a z zs).add (hB z zs),
  },
end
...
theorem fast_series_converge {f : ℕ → ℂ → ℂ} {s : set ℂ} {c a : ℝ}
  ...
  constructor, {
    exact uniform_analytic_lim o (finite_sums_are_analytic h) su,
  }, {
    exact λ z zs, summable.has_sum (fast_series_converge_at a0 a1 (λ n, hf n z zs)),
  }
end
